...
<form action="" name="search_final_master" method="post">
...
<div class="col-md-3 col-sm-12  form-group"><label>Select Master :</label>
<select name="search_party" class="form-control autosel" >
<option value="">-Please Select-</option>
<?php foreach($tbl_master as $key => $val){
	echo '<option value="'.$val['id'].'" '.(($this->input->post('search_party')==$val['id']) ? 'selected' :'').'>'.$val['admin_name'].'</option>';
}?>
</select>
</div>
<!--<div class="col-md-3 col-sm-12  form-group"><label>Select Agent :</label>
<select name="agent_name" class="form-control autosel" >
<option value="">-Please Select-</option>
        ...
        <tbody>
           <?php foreach($tbl_openno as  $key=> $val){ //echo '<pre>'; print_r($val); echo '</pre>'; die;
			 ...
			 ?>
			  ...
			  <td><?=round($val['opening_bal'])?></td>
				
				<!--<td><?=$val['hisab']?></td>-->
				 <td><?=round($val['today_hisab'])?></td>
				 <td><?=round($val['voucher'])?></td>
				 <td><?=round($val['final_hisab'])?></td>
				 <!--<td><?=$ttoday ?>-->
			   <td>
