...
$commission = ceil($tamount*((float)$val['ledgerdara_commision']/100));
...
//echo $pattiamnt;
$todayDated = date('d-m-Y',strtotime($tdledger['today'][0]['Date']));
$data= get_ledger_entry($todayDated, $_POST['ledger_id']);
//pr($data);
$bopening = $opening;
$todayDate = date('Y-m-d',strtotime($tdledger['today'][0]['Date']));
...
<!-- <td><?=get_latest_balance_till_date($_POST['ledger_id'],$yesterday)?></td> -->
<td><?=$data[0]['opening_bal']?></td>
...
<td><?=get_latest_coin_balance_till_selected_date($_POST['ledger_id'],$todayDate)?></td>
<!-- <td><?=$final_hisab ?></td> -->
