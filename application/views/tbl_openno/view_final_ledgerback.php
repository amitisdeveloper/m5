...
<table id="example" class="ui celled table cell-border" style="width:100%">
        ...
        <tbody>
           <?php foreach($tbl_openno as  $key=> $val){ //echo '<pre>'; print_r($val); echo '</pre>'; //die;
			...
			//if((($val['Open_Number_amount'])!='0')||(($val['Akhar_Number_amount'])!='0')){
...
//crontoupdateclosing
			  ...
			  <!-- <td><?=$val['todayhisab']?></td> -->
				<!--<td><?=(isset($val['openingbal'][0]['opening']))?$val['openingbal'][0]['opening']:$val['openingbal']?></td>-->
				<td><?=$opening = (isset($val['openingbal'][0]['opening']))?$val['openingbal'][0]['opening']:$val['openingbal']?></td>
				<!--<td><?=$val['hisab']?></td>-->
				 <td><?=$val['todayhisab']?></td>
				 <td><?=$opening + $val['todayhisab']?></td>
				 <!--<td><?=$ttoday ?>-->
			   <td>
...
if($opening=='0'){			  
  ...
  if($opening=='0'){
	 // $result= $CI->Tbl_openno_model->update_opening($ledger_id,$date,$opening); 
	  echo '<td>'.$ledger_id.' '.$date.'</td><br>';
  }
			//}
		   } ?>
		   </tr>
