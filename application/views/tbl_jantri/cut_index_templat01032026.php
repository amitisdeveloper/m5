...
function mapToBoxKey($n): int {
    $s = trim((string)$n);
    if ($s === '00' || $s === '0' || $s === '000' || $s === '100') return 100;
...
function distributeAmount($number, $amount, &$table, $dcomm=0, $dmcomm=0, $dhissa=0, $tppercentage=0) {
    $amount       = toFloatSafe($amount);
    $dcomm        = toFloatSafe($dcomm);
    $dmcomm       = toFloatSafe($dmcomm);
    $dhissa       = toFloatSafe($dhissa);
    $tppercentage = toFloatSafe($tppercentage);

    $adjusted = $amount;

    $comm = $dcomm + $dmcomm;
    if ($comm > 0)         $adjusted -= ($comm * $adjusted / 100);
    if ($dhissa > 0)       $adjusted -= ($dhissa * $adjusted / 100);
    if ($tppercentage > 0) $adjusted -= ($tppercentage * $adjusted / 100);

    if ($adjusted < 0) $adjusted = 0;

    $key = mapToBoxKey($number);
    if ($key === -1) return;

    $table[$key] += $adjusted; // ✅ adds: both "00" go into $table[100]
}

foreach ($tbl_transactions as $entry) {
    $numbers = array_map('trim', explode(',', $entry['trnno']));
    $amounts = array_map('trim', explode(',', $entry['trn_amt']));
...
">
    ...
    ?>
    <td>
        <span style="text-align:center;float: left;margin-right: 10px;margin-left: 5px;">
            <?= ($i == 100) ? '00' : sprintf('%02d', $i) ?>
        </span>
