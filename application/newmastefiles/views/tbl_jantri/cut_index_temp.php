...
function distributeAmount($number, $amount, &$table, $dcomm = 0, $dhissa = 0, $tppercentage = 0) {
   ...
   if (!is_numeric($amount) || !is_numeric($dcomm) || !is_numeric($dhissa) || !is_numeric($tppercentage)) {
    log_message('error', 'Invalid input in distributeAmount | ' . json_encode([
        'number' => $number,
        'amount' => $amount,
        'dcomm' => $dcomm,
        'dhissa' => $dhissa,
...
if (!empty($tbl_transactions)) {
    ...
    foreach ($tbl_transactions as $entry) { //pr($entry);
        ...
        foreach ($numbers as $index => $number) {
            if (isset($amounts[$index])) {
                ...
                $comm = (float)($entry['dcomm'] ?? 0) + (float)($entry['dmcomm'] ?? 0);

                distributeAmount($number, $amount, $table,$comm , $entry['dhissa'], $entry['tppercentage']);
            }
        }
    }
}
