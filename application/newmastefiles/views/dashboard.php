            ...
            <div class="row mt-4">
                <?php foreach ($tbl_openno as $key => $val) { ?>  
                    <div class="col-sm-4 mb-3">
                        <div class="p-3 text-white rounded dashboard-tile dashboard-section shift-box" data-json='<?=json_encode($val)?>' data-bs-toggle="modal" data-bs-target="#editModal">
                            <h6 class="text-3d"><i class="fas fa-clock mr-2"></i><?= $val['shift_name'] ?></h6>
                            <h6>Date : <?= $val['date'] ?></h6>
...
<script>
 ...
 <!-- Edit Modal -->
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h6 class="modal-title" id="editModalLabel">Edit Result</h6>
                    <button type="button" class="btn btn-primary btn-sm create-btn">Create Result</button>
                ...
                <div class="modal-body" id="editModalBody">
                    <form id="editForm">
                        <div class="form-group">
                            ...
                            <select name="shift_name" class="form-control" required="required">
                                <option value="">Select Shift</option>
                                <option value="Delhi Bazaar">Delhi Bazaar</option>
                        ...
                        <div class="form-group">
                            ...
                            <input id="editDate" class="form-control hasDatepicker" name="date" type="text" autocomplete="off" required="required">
                        </div>
                        <div class="form-group">
                            <label>Result <span class="required">*</span></label>
                            <input  class="form-control" name="time" type="text" autocomplete="off" required="required">
                        </div>
    ...
    <div class="modal fade" id="createModal" tabindex="-1" role="dialog" aria-labelledby="createModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h6 class="modal-title" id="createModalLabel">Create Shift</h6>
                    <button type="button" class="btn btn-secondary btn-sm back-btn">Back</button>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">×</span>
                    </button>
                </div>
                <div class="modal-body" id="createModalBody">
                    <form id="createForm">
                        ...
                        <div class="form-group">
                            <label>Date <span class="required">*</span></label>
                            <input id="createDate" class="form-control hasDatepicker" name="date" type="text" autocomplete="off" required="required">
                        </div>
                        <div class="form-group">
                            <label>Result <span class="required">*</span></label>
                            <input  class="form-control" name="time" type="text" autocomplete="off" required="required">
 ...
 // Store edit modal form data
        let editFormData = {};

        // Handle shift box clicks
        $(document).on('click', '.shift-box', function() {
            const jsonData = $(this).data('json');
//console.log(jsonData);
            // Populate edit modal form
            $('#editForm [name="shift_name"]').val(jsonData.shift_name || '');
            $('#editForm [name="date"]').val(jsonData.date || '');
            $('#editForm [name="time"]').val(jsonData.time || '');

            // Store form data
            editFormData = {
                shift_name: jsonData.shift_name || '',
                date: jsonData.date || '',
                time: jsonData.time || ''
            };
        });

        // Handle create button in edit modal
        $(document).on('click', '.create-btn', function(e) {
            e.preventDefault();
            // Reset create modal form
            $('#createForm')[0].reset();
            // Show createModal without closing editModal
            $('#createModal').modal('show');
            // Add backdrop for stacking
            $('body').addClass('modal-open');
            $('.modal-backdrop').last().css('z-index', 1050);
            $('#createModal').css('z-index', 1060);
        });

        // Handle back button in create modal
        $(document).on('click', '.back-btn', function() {
            // Restore edit modal form data
            $('#editForm [name="shift_name"]').val(editFormData.shift_name);
            $('#editForm [name="date"]').val(editFormData.date);
            $('#editForm [name="time"]').val(editFormData.time);

            // Hide create modal and ensure edit modal remains visible
            $('#createModal').modal('hide');
            $('#editModal').addClass('show').css('display', 'block');
            // Adjust backdrop for editModal
        ...
        $(document).on('click', '.close', function() {
            ...
            // Ensure backdrop is removed if no modals are open
...
.dashboard-tile, .stretched-link {
    cursor: pointer;
    transition: transform 0.2s ease-in-out; /* Smooth transition for scaling */
}
.dashboard-tile:hover, .stretched-link:hover {
